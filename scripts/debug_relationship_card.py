...
def main() -> int:
    ...
    had_error = False
    try:
        payload = asyncio.run(_run_skill("relationship", symbol, company_name))
    except Exception as exc:  # pragma: no cover - debug fallback
        from scripts.debug_skill import build_error_payload

        payload = build_error_payload("relationship", symbol, company_name, exc)
        had_error = True
    write_skill_output(payload, default_output_path("relationship"))
