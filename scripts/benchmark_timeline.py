...
from pomefi.stock_wiki.skills.timeline import _load_events_branch
...
OUTPUT_PATH = PROJECT_ROOT / "debug_outputs" / "stock_wiki" / "timeline.kimi.benchmark.json"


def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Benchmark the timeline Kimi events branch.")
...
def _summarize_events(events: list[dict[str, Any]]) -> list[str]:
    ...
    for item in events:
        ...
        if not title:
            continue
        headline = title if not content else f"{title} | {content}"
        parts = [part for part in (date, headline, sentiment) if part]
...
def _sanitize_trace(trace: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "evidence_preview": str(trace.get("final_content") or ""),
...
async def _run_benchmark(symbol: str, company_name: str, runs: int) -> dict[str, Any]:
    ...
    if not config.api_key:
        raise RuntimeError("timeline benchmark requires KIMI_API_KEY")
        ...
        for index in range(1, runs + 1):
            ...
            try:
                with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
                    branch = await _load_events_branch(
                        symbol,
                        company_name,
                        config=config,
    ...
    event_counts = [len(list(row["events"])) for row in success_rows]
    summary = {
        "runs": runs,
        ...
        "avg_events": round(statistics.mean(event_counts), 2) if event_counts else 0.0,
    }
    return {
        "skill": "timeline_kimi_benchmark",
        "symbol": symbol,
